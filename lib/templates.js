...
function createDirs(host, owner, repo, template) {
  const dirs = {
    templates: path.join(os.homedir(), '.rig/templates')
  }

  if (owner) dirs.owner = path.join(dirs.templates, host, owner)
...
async function checkoutVersion(host, owner, repo, template, version) {
  ...
  const { name } = tmp.dirSync({ unsafeCleanup: true })

  const tag = version !== 'master' ? `tags/${template}#${version}` : 'master'
